...
namespace asp_NguyenVanBac.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var lstProduct = objwebsiteBHEntities.Products.ToList();
        ...
        public ActionResult Register(User _user)
        {
            ...
            {
                var check = objwebsiteBHEntities.Users.FirstOrDefault(s => s.Email == _user.Email);
                if (check == null)
                {
                    _user.Password = GetMD5(_user.Password);
                    objwebsiteBHEntities.Configuration.ValidateOnSaveEnabled = false;
                    objwebsiteBHEntities.Users.Add(_user);
        ...
        public ActionResult Login(string email, string password)
        {
            if (ModelState.IsValid)
            {
                ...
                var data = objwebsiteBHEntities.Users.Where(s => s.Email.Equals(email) && s.Password.Equals(f_password)).ToList();
                if (data.Count() > 0)
                {
                    //add session
